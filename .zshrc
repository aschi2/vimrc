...
#iterm2 shell itegration nonsense
case "$OSTYPE" in
  darwin*)
...
function prompt_screen() {
  if [ -n "$STY" ]; then p10k segment -b 'green' -f 3 -t "In screen ${STY}"; fi
  if [ -n "$TMUX" ]; then p10k segment -b 'green' -f 3 -t "In tmux ${TMUX}"; fi
}
#nvim
alias vim=nvim
alias vi=nvim
