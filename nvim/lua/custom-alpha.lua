...
local default_header = {
	type = "text",
	val = read_header_from_file("~/vimrc/.splash"),
	opts = {
		hl = "ErrorMsg",
		shrink_margin = false,
		position = 'center',
...
local function mru(start, cwd, items_number, opts)
	...
	for i, fn in ipairs(oldfiles) do
		...
		if #short_fn > 45 then
		short_fn = string.sub(short_fn, -45)
		end
		local file_button_el = file_button(fn, tostring(i + start - 1),string.format("%-45s",short_fn), opts.autocd)
